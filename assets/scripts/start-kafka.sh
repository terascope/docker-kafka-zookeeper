...
if [ ! -z "$ZK_CHROOT" ]; then
    ...
    # configure kafka
    sed -r -i "s/(zookeeper.connect)=(.*)/\\1=localhost:$ZOOKEEPER_PORT\\/$ZK_CHROOT/g" "$KAFKA_HOME/config/server.properties"
else
    # configure kafka
    sed -r -i "s/(zookeeper.connect)=(.*)/\\1=localhost:$ZOOKEEPER_PORT/g" "$KAFKA_HOME/config/server.properties"
fi
